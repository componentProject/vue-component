...
import { addClass, browse, getEventTargetNode, getPaddingTopBottomSize, hasClass, isNodeElement, isPx, isScale, removeClass, setScrollLeft, setScrollTop } from '../../ui/src/dom'
import { errLog, warnLog } from '../../ui/src/log'
import { eqEmptyValue, formatText, getFuncText, getLastZIndex, hasChildrenList, isEnableConf, nextZIndex } from '../../ui/src/utils'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const getOrderField = (column: VxeTableDefines.ColumnInfo) => {
      ...
      return (row: any) => {
        ...
        if (!sortType || sortType === 'auto') {
          return Number.isNaN(cellValue) ? cellValue : XEUtils.toNumber(cellValue)
        }
        ...
        if (!allRemoteFilter && filterColumns.length) {
          const handleFilter = (row: any) => {
            return filterColumns.every(({ column, valueList, itemList }) => {
              ...
              return valueList.includes(XEUtils.get(row, column.field))
            })
          }
              ...
              if (fullColumnIdData[colid]) {
                ...
                if (listElem) {
                  XEUtils.arrayEach(listElem.querySelectorAll(`.${column.id}`), (elem: any) => {
                    const colspan = Number.parseInt(elem.getAttribute('colspan') || 1)
    ...
    const debounceScrollY = XEUtils.debounce((evnt: Event) => {
    ...
    const handleTooltip = (evnt: MouseEvent, cell: HTMLTableCellElement, overflowElem: HTMLElement, tipElem: HTMLElement | null, params: any) => {
      ...
      const content = useCustom ? customContent : XEUtils.toString(column.type === 'html' ? overflowElem.textContent : overflowElem.textContent).trim()
