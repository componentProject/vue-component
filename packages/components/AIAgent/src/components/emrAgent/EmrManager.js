...
import { updateEmrData, initEmrObject, parseEmrData, clearEmrData } from './js/syncMonacoData';
import { generateEmr, completeLine } from '../api/api';
...
export class EmrManager {
    ...
    async createEmr(jsonData, agentId, showChat) {
        this.state.currentAgentId = agentId;
        if (!jsonData || !agentId || !this.state.emr_data.normal) return;
    ...
    async generateEmr(aiData, agentId, showChat) {
        ...
        try {
            if (showChat) this.context.isChatVisible = true;
