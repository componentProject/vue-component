<template>
    ...
    >
        ...
        <div class="floating-button-content" v-else>
            <i class="ai-iconfont icon-robot floating-button-icon"></i>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getRect() {
            // 添加安全判断
            if (!this.$refs.floatingButton) return null;
        ...
        handleDragBounds(leftPx, topPx, width, height) {
            if (!this.$refs.floatingButton) return; // 添加空值判断
            // 限制左边界
            if (leftPx < 0) {
                this.leftRate = 0;
            }
