...
class ShortCut {
  ...
  paste(list: AllFormItem[]) {
    if (state.copyContent) {
      const pasteControl = flex.deepClone(state.copyContent)
      if (pasteControl.data) {
        pasteControl.data.fieldName = `${pasteControl.ControlType}_${flex.generateMixed()}`
        pasteControl.id = flex.generateMixed()
