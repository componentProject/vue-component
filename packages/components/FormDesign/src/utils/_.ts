...
class Flex {
   ...
   * json转标准数据格式进行收口
   * @param item - 要转换的表单项
   * @param formcomponents - 表单组件映射（可选，如果不提供则使用空对象）
   */
  public jsonToForm(item: AllFormItem, formcomponents?: Record<string, any>) {
    if (!item.data || !item.controlItems) {
      item = this.deepClone(item)
      // 优先使用传入的 formcomponents，如果没有提供则使用空对象
      // 注意：formcomponents 必须通过 FormDesignStore 传入
      const formcomps = formcomponents || {}
      const currentComponent = formcomps[item.ControlType as any]
      item.formConfig = formcomps[item.ControlType as any]?.formConfig || {}
      if (!item.data) {
          ...
          item.data.trs = item.data.trs.map((trItem: any) => {
            trItem.tds.forEach((tdItem: any) => {
              if (tdItem.list && tdItem.list.length > 0) {
                tdItem.list = this.jsonToForm(tdItem.list, formcomponents)
              }
              return tdItem
            })
            return trItem
