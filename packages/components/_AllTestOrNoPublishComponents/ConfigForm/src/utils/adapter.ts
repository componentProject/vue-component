 ...
 * 3. adapter.inferDataType（自定义推断方法）
 * 4. fields[type].dataType（Adapter 完整格式配置）
 * 5. adapter.dataTypeMap[type]（Adapter 映射表）
 * 6. DEFAULT_DATA_TYPE_MAP[type]（默认映射表）
 * 7. 'any'（最终兜底）
 *
 * @param field - 字段配置
 ...
 *
 * // 结构推断
 * inferDataType({ type: 'tabs', items: {...} }, adapter)  // => 'array'
 *
 * // 默认推断
...
export function inferDataType(field: FieldConfig, adapter: UIAdapter): DataType {
  ...
  if ('properties' in field && field.properties != null) {
    // 区分 object 数据类型和 void 布局类型
    const voidTypes = ['void', 'group', 'card', 'tabs', 'collapse']
    if (voidTypes.includes(field.type)) {
      return 'void'
    }
    return 'object'
  }

  // 3. 如果 Adapter 提供了自定义推断方法，调用它
  if (adapter.inferDataType) {
    return adapter.inferDataType(field)
  }

  // 4. 从 Adapter 的 fields 配置中获取（完整格式）
