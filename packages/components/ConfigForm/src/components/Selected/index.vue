...
<template>
  <div class="starfish-formitem" :class="{ 'formCover': drag, 'starfish-vertical': labelalign != 'top', [item.data.csslist?.join(' ')]: !!item.data.csslist }">
    ...
    <div class="control" :style="{ marginLeft: labelalign != 'top' ? `${labelWidth}px` : '' }">
      <ElSelect v-if="drag" v-model="item.data.itemConfig.value" :placeholder="item.data.placeholder" :size="size">
        <ElOption v-for="items in item.data.itemConfig.items" :key="items.value" :label="items.label" :value="items.value" />
      </ElSelect>
      <ElSelect v-if="!drag" v-model="data[item.data.fieldName]" :placeholder="item.data.placeholder" :size="size" @focus="execFunc('onFocus')" @blur="execFunc('onBlur')">
        <ElOption v-for="items in item.data.itemConfig.items" :key="items.value" :label="items.label" :value="items.value" />
      </ElSelect>
    </div>
  </div>
