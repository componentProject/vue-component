...
<template>
  <div class="starfish-formitem" :class="{ 'formCover': drag, 'starfish-vertical': labelalign !== 'top', [item.data.csslist?.join(' ')]: !!item.data.csslist }">
    ...
    <div class="control" :style="{ marginLeft: labelalign !== 'top' ? `${labelWidth}px` : '' }">
      <ElInputNumber v-if="drag" v-model="item.data.default" :controls-position="item.data.type === 2 ? 'right' : ''" :size="item.data.size" />
      <ElInputNumber v-if="!drag" v-model="data[item.data.fieldName]" :controls-position="item.data.type === 2 ? 'right' : ''" :size="item.data.size" @focus="execFunc('onFocus')" @blur="execFunc('onBlur')" />
    </div>
  </div>
</template>
...
import { ElIcon, ElInputNumber, ElTooltip } from 'element-plus'
