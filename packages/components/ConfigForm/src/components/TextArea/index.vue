<template>
  <div class="starfish-formitem" :class="{ 'formCover': drag, 'starfish-vertical': labelalign != 'top', [item.data.csslist?.join(' ')]: !!item.data.csslist }">
    ...
    <div class="control" :style="{ marginLeft: labelalign != 'top' ? `${labelWidth}px` : '' }">
      <ElInput v-if="drag" v-model="item.data.default" type="textarea" :placeholder="item.data.placeholder" :size="size" />
      <ElInput v-if="!drag" v-model="data[item.data.fieldName]" type="textarea" :placeholder="item.data.placeholder" :size="size" @focus="execFunc('onFocus')" @blur="execFunc('onBlur')" />
    </div>
  </div>
</template>
