  ...
  methods: {
    setConfigByProp(prop, value, defaultKeyOrKey = true) {
      ...
      rows.forEach(row => {
        ...
        if (formItem?.config) {
          const { type } = formItem;
          const key = getType(defaultKeyOrKey) === 'boolean' ? typeDefaultMap[type] : defaultKeyOrKey;
          if (key) {
      ...
      <el-form ref="form" class="p8" labelSuffix="：" {...formOptions}>
        {
          this.$slots.default
            ...
            : [
                ...rows.map(row => {
                  //代表这一行隐藏
                  if (row.hidden && typeof row.hidden === 'function' && row.hidden(formOptions.model)) {
                    return [<span />];
                  }
                  //代表这一行自定义渲染
                  else if (row.render) {
                    ...
                    return [
                      <el-row {...rowProps}>
                        {_formItems.map(formItem => {
                          let { render, renderSlot, colConfig, type, config, renderLabel, ...formItemProps } = formItem;
                          ...
                          // 传递type使用内置组件
                          if (type) {
                            const { _component } = getComponent(type, config, formItemProps.prop, formOptions.model);
