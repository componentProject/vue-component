<template>
  <div class="low-code-editor" ref="editorRef">
    ...
    <div class="editor-main flex-1 flex overflow-hidden">
      <!-- 左侧组件面板 -->
      <div class="editor-sidebar w-64 bg-white border-r border-gray-200" v-show="editorMode === 'edit'">
        <component-panel
          :draggable="true"
      ...
      <!-- 右侧属性面板 -->
      <div class="editor-sidebar w-80 bg-white border-l border-gray-200" v-show="editorMode === 'edit'">
        <property-panel
 ...
 * 处理添加组件
 */
const handleAddComponent = (componentId: string, position: { left: number; top: number }, parentId?: string) => {
  try {
 ...
 * 处理更新组件样式
 */
const handleUpdateComponentStyle = (componentId: string, style: Partial<CanvasComponent['style']>) => {
  try {
